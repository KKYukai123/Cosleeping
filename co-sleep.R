...
# Export nested_data to excel
install.packages("writexl")
library(writexl)
write_xlsx(nested_data, "nested_data.xlsx")
...
ggplot(avg_sleep_time, aes(x = Age_group, y = avg_sleeps_longest_stretch_time, fill = where_sleep_longest)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label = avg_sleeps_longest_stretch_time), vjust = 1, position = position_dodge(0.9), size = 3) +
  scale_fill_brewer(palette = "Set1") +
  labs(title = "Average Sleeps Longest Stretch Time by Sleep Location and Age Group",
       x = "Age Group",
       y = "Average Sleeps Longest Stretch Time (HH:MM)",fill = "Sleep Location") +
  theme_minimal() +
  theme(plot.margin = margin(10, 10, 10, 10))

# Plotting for Baby Sleep Time
ggplot(avg_sleep_time, aes(x = Age_group, y = avg_baby_sleep_time, fill = where_sleep_longest)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label = avg_baby_sleep_time), vjust = 1, position = position_dodge(0.9), size = 3) +
  scale_fill_brewer(palette = "Set1") +
  labs(title = "Average Baby Sleep Onset Time by Sleep Location and Age Group",
       x = "Age Group",
       y = "Average Baby Sleep Time (HH:MM)",fill = "Sleep Location") +
  theme_minimal() +
  theme(plot.margin = margin(10, 10, 10, 10))

# Plotting for Baby Wake Time
ggplot(avg_sleep_time, aes(x = Age_group, y = avg_baby_wake_time, fill = where_sleep_longest)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label = avg_baby_wake_time), vjust = 1, position = position_dodge(0.9), size = 3) +
  scale_fill_brewer(palette = "Set1") +
  labs(title = "Average Baby Wake Time by Sleep Location and Age Group",
       x = "Age Group",
       y = "Average Baby Wake Time (HH:MM)",fill = "Sleep Location") +
  theme_minimal() +
  theme(plot.margin = margin(10, 10, 10, 10))

# Plotting for Baby Total Sleep Time
ggplot(avg_sleep_time, aes(x = Age_group, y = avg_baby_total_sleep_time, fill = where_sleep_longest)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label = avg_baby_total_sleep_time), vjust = 1, position = position_dodge(0.9), size = 3) +
  scale_fill_brewer(palette = "Set1") +
  labs(title = "Average Baby Total Sleep Time by Sleep Location and Age Group",
       x = "Age Group",
       y = "Average Baby Total Sleep Time (HH:MM)",fill = "Sleep Location") +
  theme_minimal() +
  theme(plot.margin = margin(10, 10, 10, 10))

# Plotting for Past Week Average Woken Up
ggplot(avg_sleep_time, aes(x = Age_group, y = avg_past_week_average_woken_up, fill = where_sleep_longest)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label = sprintf("%.2f", avg_past_week_average_woken_up)), vjust = 1, position = position_dodge(0.9), size = 3) +
  scale_fill_brewer(palette = "Set1") +
  labs(title = "Average Times Woken Up Over the Past Week by Sleep Location and Age Group",
       x = "Age Group",
       y = "Average Times Woken Up",fill = "Sleep Location") +
  theme_minimal() +
  theme(plot.margin = margin(10, 10, 10, 10))
