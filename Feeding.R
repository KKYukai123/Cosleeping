...
filtered_data$feeding_type <- "Exclusively breastfed"
...
plot_list <- lapply(plot_vars, function(var) {
  ggplot(combined_data, aes(x = feeding_type, y = combined_data[[var]] * 60)) + # Multiply by 60 to convert minutes to seconds
    geom_boxplot(outlier.shape = NA) +  # Remove outliers by setting outlier.shape to NA
    labs(y = var, title = paste(var)) +
    theme_minimal() +
    scale_y_continuous(labels = function(x) sprintf('%02d:%02d', floor(x / 3600), floor((x %% 3600) / 60))) # Format the y-axis labels as HH:MM
})
